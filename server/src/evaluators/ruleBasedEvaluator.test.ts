    ...
    requiredBehaviors: [
      {
        ...
        keywords: ["park", "assign", "allocate", "available"],
        description: "The system should allocate a parking spot.",
      },
      {
        name: "Ticket generation",
        keywords: ["ticket", "entry", "checkin"],
        description: "The system should create a ticket.",
      },
      {
        name: "Fee calculation",
        keywords: ["fee", "price", "payment", "calculate"],
        description: "The system should calculate fees.",
      },
    ],
    recommendedConcepts: ["Vehicle abstraction", "Fee strategy"],
  },
...
const completeSubmission = {
  classes: [
    {
      name: "ParkingLot",
      responsibility: "Coordinates floors and parking operations.",
      methods: ["findAvailableSpot", "parkVehicle"],
    },
    {
      name: "ParkingFloor",
      responsibility: "Tracks parking spots on a floor.",
      methods: ["findAvailableSpot"],
    },
    {
      name: "ParkingSpot",
      responsibility: "Represents an individual parking location.",
    ...
    {
      name: "Ticket",
      responsibility: "Tracks vehicle entry and exit information.",
      methods: ["close"],
    },
    {
      name: "Payment",
      responsibility: "Represents payment for parking.",
      methods: ["calculateFee", "pay"],
    },
  ],
  relationships: [
    {
...
describe("RuleBasedEvaluator", () => {
  ...
  it("detects missing required entities", async () => {
    ...
    const submission = {
      ...completeSubmission,
      classes: completeSubmission.classes.slice(0, 3),
    } as unknown as ISubmission;
  ...
  it("detects missing required behavior", async () => {
    ...
    const submission = {
      ...completeSubmission,
      classes: completeSubmission.classes.map((item) => ({
        ...item,
        methods: item.methods.filter(
          (method) => !method.toLowerCase().includes("park")
        ),
      })),
      explanation:
        "The design separates responsibilities between domain classes.",
    } as unknown as ISubmission;
    ...
    expect(result.issues.some(
      (issue) =>
        issue.title === "Missing behavior: Parking spot allocation"
    )).toBe(true);
  });
  ...
  it("flags weak design reasoning", async () => {
    ...
    const submission = {
      ...completeSubmission,
      explanation: "I created these classes.",
    } as unknown as ISubmission;
